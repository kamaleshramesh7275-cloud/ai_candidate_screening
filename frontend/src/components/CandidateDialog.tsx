...
export function CandidateDialog({ candidate, isOpen, onClose, onNotesSaved }: CandidateDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-50">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold flex items-center gap-3">
            <span>{candidate.name}</span>
            <Badge variant="outline" className="font-normal text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800">
              {candidate.domain || 'Unspecified'}
            </Badge>
          </DialogTitle>
          <DialogDescription className="text-slate-500 dark:text-slate-450">{candidate.email}</DialogDescription>
        </DialogHeader>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 pt-4">
          <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
            <div className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Resume Score</div>
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">{candidate.resumeScore?.toFixed(1) || '—'}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
            <div className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">GitHub Score</div>
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">{candidate.githubScore?.toFixed(1) || '—'}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
            <div className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Test Score</div>
            <div className="text-2xl font-bold text-slate-800 dark:text-slate-100">{candidate.testScore?.toFixed(1) || '—'}</div>
          </div>
          <div className={`p-4 rounded-xl border ${candidate.cheatStrikes > 0 ? 'bg-red-50 dark:bg-red-950/20 border-red-100 dark:border-red-900/30' : 'bg-slate-50 dark:bg-slate-800/40 border-slate-100 dark:border-slate-800'} text-center`}>
            <div className={`text-sm font-medium mb-1 ${candidate.cheatStrikes > 0 ? 'text-red-500 dark:text-red-400' : 'text-slate-500 dark:text-slate-400'}`}>Cheat Strikes</div>
            <div className={`text-2xl font-bold ${candidate.cheatStrikes > 0 ? 'text-red-700 dark:text-red-300' : 'text-slate-800 dark:text-slate-100'}`}>{candidate.cheatStrikes || 0}</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="bg-slate-900 dark:bg-slate-950 rounded-xl p-4 h-56 overflow-auto font-mono text-xs">
              {candidate.cheatLog && JSON.parse(candidate.cheatLog).length > 0 ? (
                <ul className="space-y-2">
                  {JSON.parse(candidate.cheatLog).map((log: any, i: number) => (
                    <li key={i} className="text-red-400 border-b border-slate-800 dark:border-slate-900 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
          ...
          <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-850 p-4 border border-slate-200 dark:border-slate-800 rounded-xl h-56 overflow-auto">
                     ...
                     <div>
                       ...
                       <div className="flex flex-wrap gap-2">
                         {parsedLog.missing?.map((skill: string) => (
                           <Badge key={skill} variant="outline" className="text-slate-500 dark:text-slate-400 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900">{skill}</Badge>
                         )) || <span className="text-xs text-slate-400">None</span>}
                       </div>
                     </div>
                   </div>
          ...
          <div>
            ...
            <div className="bg-slate-50 dark:bg-slate-850 p-4 border border-slate-200 dark:border-slate-800 rounded-xl min-h-56 overflow-auto space-y-4">
                ...
                return (
                  <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-2">
                      <div className="bg-white dark:bg-slate-900 rounded-lg p-2 border border-slate-200 dark:border-slate-800 text-center">
                        <div className="text-lg font-black text-slate-800 dark:text-slate-100">{gh.public_repos ?? '—'}</div>
                      ...
                      <div className="bg-white dark:bg-slate-900 rounded-lg p-2 border border-slate-200 dark:border-slate-800 text-center">
                        <div className="text-lg font-black text-emerald-600 dark:text-emerald-450">{gh.recentCommitsScore ?? '—'}</div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {langEntries
                            ...
                            .slice(0, 10)
                            .map(([lang, count]) => (
                              <span key={lang} className="px-2 py-0.5 rounded-md bg-indigo-100 dark:bg-indigo-950 text-indigo-750 dark:text-indigo-400 text-xs font-semibold">
                    ...
                    {activity && Object.keys(activity).length > 0 && (
                      <div className="pt-2 border-t border-slate-200 dark:border-slate-800">
                        ...
                        <div className="flex gap-1.5 items-end justify-between bg-white dark:bg-slate-900 p-2.5 rounded-lg border border-slate-105 dark:border-slate-800">
                          {Object.entries(activity).map(([month, count]: any) => {
                            const countNum = Number(count) || 0;
                            let color = 'bg-slate-105 dark:bg-slate-800 hover:bg-slate-200';
                            if (countNum > 0 && countNum <= 2) color = 'bg-emerald-100 dark:bg-emerald-950/40 hover:bg-emerald-200';
                            else if (countNum > 2 && countNum <= 5) color = 'bg-emerald-300 dark:bg-emerald-900/60 hover:bg-emerald-450';
                  ...
                  <div className="space-y-3">
                    {data.headline && (
                      <div>
                        ...
                        <div className="text-sm font-semibold text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-900 rounded-lg p-2.5 border border-slate-200 dark:border-slate-800">{data.headline}</div>
                      </div>
                    )}
                    {data.summary && (
                      <div>
                        <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Summary</div>
                        <div className="text-sm text-slate-700 dark:text-slate-350 bg-white dark:bg-slate-900 rounded-lg p-2.5 border border-slate-200 dark:border-slate-800 leading-relaxed">{data.summary}</div>
                      </div>
                    )}
                    ...
                    {candidate.linkedInUrl && (
                      <a href={candidate.linkedInUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-blue-650 dark:text-blue-400 hover:underline mt-1 font-semibold">
          ...
          <div className="md:col-span-2 mt-2">
            ...
            <div className="bg-slate-50 dark:bg-slate-850 p-5 border border-slate-200 dark:border-slate-800 rounded-xl space-y-4">
              {candidate.resumeText ? (() => {
                ...
                return (
                  <div className="space-y-4">
                    <div className="flex flex-wrap gap-2 pb-3 border-b border-slate-250 dark:border-slate-800">
                      {keywords.map(kw => {
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800 max-h-60 overflow-auto text-xs text-slate-600 dark:text-slate-400 font-mono whitespace-pre-wrap leading-relaxed">
          ...
          <div className="md:col-span-2 border-t border-slate-200 dark:border-slate-800 pt-4 mt-2">
            ...
            <div className="flex gap-3 items-start">
              <textarea
                rows={3}
                className="flex-1 min-h-[85px] text-sm p-3 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none bg-slate-50 dark:bg-slate-850 focus:bg-white dark:focus:bg-slate-900 transition-all resize-none shadow-inner text-slate-800 dark:text-slate-100"
