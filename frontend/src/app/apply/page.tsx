  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col md:flex-row transition-colors duration-200">
      ...
      <div className="md:w-2/3 p-8 md:p-16 flex items-center justify-center">
        {!intakeResult ? (
          <Card className="w-full max-w-xl border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-none rounded-2xl overflow-hidden bg-white dark:bg-slate-900">
            <div className="h-2 w-full bg-gradient-to-r from-blue-600 to-indigo-600"></div>
            <CardHeader className="pb-4">
              <CardTitle className="text-2xl font-bold text-slate-800 dark:text-slate-100">Candidate Intake</CardTitle>
              <CardDescription className="text-slate-500 dark:text-slate-400">All fields are required for an accurate score.</CardDescription>
            </CardHeader>
            <form onSubmit={handleSubmit}>
              <CardContent className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-slate-700 dark:text-slate-350 font-medium">Full Name</Label>
                    <div className="relative">
                      <User className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
                      <Input 
                        id="name" 
                        required 
                        className="pl-9 bg-slate-50 dark:bg-slate-850 border-slate-200 dark:border-slate-700 focus-visible:ring-blue-500 text-slate-900 dark:text-slate-50"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-slate-700 dark:text-slate-350 font-medium">Email Address</Label>
                    <Input 
                      id="email" 
                      type="email" 
                      required 
                      className="bg-slate-50 dark:bg-slate-850 border-slate-200 dark:border-slate-700 focus-visible:ring-blue-500 text-slate-900 dark:text-slate-50"
                ...
                <div className="space-y-2">
                  <Label htmlFor="domain" className="text-slate-700 dark:text-slate-350 font-medium">Target Role / Domain</Label>
                  <Select required onValueChange={(value: string | null) => setFormData({...formData, domain: value || ''})}>
                    <SelectTrigger className="bg-slate-50 dark:bg-slate-850 border-slate-200 dark:border-slate-700 focus:ring-blue-500 text-slate-900 dark:text-slate-50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-2">
                    <Label htmlFor="linkedin" className="text-slate-700 dark:text-slate-350 font-medium">LinkedIn URL</Label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="github" className="text-slate-700 dark:text-slate-350 font-medium">GitHub URL</Label>
                    <div className="relative">
                ...
                <div className="space-y-2 pt-2">
                  <Label htmlFor="resume" className="text-slate-700 dark:text-slate-350 font-medium">Upload Resume (PDF only)</Label>
                  <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-6 flex flex-col items-center justify-center text-center bg-slate-50 dark:bg-slate-850 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors relative cursor-pointer">
                    <Input 
                      id="resume" 
                      type="file" 
                      accept=".pdf"
              ...
              <CardFooter className="bg-slate-50/50 dark:bg-slate-850/50 pt-4 px-6 pb-6 mt-4">
                <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-200 dark:shadow-none h-12 text-lg rounded-xl transition-all" disabled={loading || !formData.domain}>
                  {loading ? (
          ...
          <Card className="w-full max-w-xl border-slate-200 dark:border-slate-800 shadow-xl rounded-2xl overflow-hidden bg-white dark:bg-slate-900 p-8 flex flex-col items-center">
            <div className="bg-emerald-100 dark:bg-emerald-950 p-4 rounded-full text-emerald-600 dark:text-emerald-400 mb-6">
              <CheckCircle2 className="w-12 h-12" />
            </div>
            
            <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2 text-center">Profile Verified!</h2>
            ...
            <div className="grid grid-cols-2 gap-4 w-full mb-8">
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Resume Score</div>
                <div className="text-3xl font-black text-blue-600 dark:text-blue-400">{intakeResult.scores.resume.toFixed(0)}%</div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 text-center">
                <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">GitHub Activity</div>
                <div className="text-3xl font-black text-purple-600 dark:text-purple-400">{intakeResult.scores.github.toFixed(0)}%</div>
              </div>
            </div>
            ...
            <div className="w-full space-y-4">
              ...
              <Button onClick={handleStartTest} className="w-full bg-blue-600 hover:bg-blue-700 text-white h-12 text-lg rounded-xl flex items-center justify-center gap-2">
