...
export default function RecruiterPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-center p-4 pt-20 relative overflow-hidden transition-colors duration-200">
        <div className="absolute top-4 right-4 z-50">
          <ThemeToggle />
        </div>
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-red-600/10 dark:bg-red-600/20 blur-[120px]"></div>
        
        <Card className="w-full max-w-md border-zinc-200 dark:border-zinc-800 shadow-xl rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 z-10">
          <div className="h-2 w-full bg-red-600"></div>
          <CardHeader className="text-center pb-4">
            <div className="mx-auto bg-red-500/10 dark:bg-red-500/20 w-12 h-12 rounded-full flex items-center justify-center mb-2">
              <KeyRound className="w-6 h-6 text-red-500" />
            </div>
            <CardTitle className="text-2xl font-bold text-zinc-800 dark:text-zinc-100">Recruiter Access</CardTitle>
            <CardDescription className="text-zinc-500 dark:text-zinc-400">Enter security password to access dashboard.</CardDescription>
          </CardHeader>
          <form onSubmit={handleLogin}>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Input 
                  ...
                  className="bg-zinc-50 dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-900 dark:text-zinc-50"
                />
              </div>
              {loginError && (
            ...
            <CardFooter className="flex flex-col gap-2">
              <Button type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white rounded-xl h-11" disabled={loginLoading}>
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 transition-colors duration-200">
      ...
      <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div 
            onClick={() => router.push('/')}
            className="flex items-center gap-2 font-bold text-xl text-zinc-800 dark:text-zinc-100 cursor-pointer"
          >
            <div className="bg-red-600 p-1.5 rounded text-white"><Users className="w-5 h-5" /></div>
            AI Recruiter
          </div>
          <div className="flex items-center gap-4">
            <div className="relative hidden md:block">
              ...
              <Input 
                ...
                className="pl-9 bg-zinc-100 dark:bg-zinc-800 border-none w-64 focus-visible:ring-1 focus-visible:ring-red-500 rounded-full text-zinc-900 dark:text-zinc-55"
                value={searchTerm}
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl">
            <CardHeader className="pb-2">
              <CardDescription className="text-sm font-medium text-zinc-500">Total Applicants</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{candidates.length}</div>
            </CardContent>
          </Card>
          <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl">
            <CardHeader className="pb-2">
              <CardDescription className="text-sm font-medium text-zinc-500">Shortlisted</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl">
            <CardHeader className="pb-2">
              <CardDescription className="text-sm font-medium text-zinc-500">Avg Score</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl">
            <CardHeader className="pb-2">
              <CardTitle className="text-md font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-red-500" /> Status Distribution
              </CardTitle>
            </CardHeader>
            <CardContent className="h-64 flex items-center">
              {statusData.length > 0 ? (
                    ...
                    {statusData.map((d) => (
                      <div key={d.name} className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-zinc-600 dark:text-zinc-400">{d.name}</span>
                        </div>
                ...
                <div className="text-zinc-400 text-sm w-full text-center italic">No records available.</div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl">
            <CardHeader className="pb-2">
              <CardTitle className="text-md font-bold text-zinc-800 dark:text-zinc-200 flex items-center gap-2">
                <Users className="w-4 h-4 text-rose-500" /> Domains Applied
              </CardTitle>
            </CardHeader>
            <CardContent className="h-64">
              {domainData.length > 0 ? (
                ...
                <div className="text-zinc-400 text-sm h-full flex items-center justify-center italic">No records available.</div>
              )}
            </CardContent>
          </Card>
              ...
              {candidates.length > 0 ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between pb-2 border-b border-zinc-100 dark:border-zinc-800">
                    <div>
                      <div className="text-4xl font-black text-zinc-800 dark:text-zinc-100">{passRateInfo.passRate.toFixed(1)}%</div>
                      <div className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider">Pass Rate (Score ≥ 70)</div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <div className="bg-zinc-50 dark:bg-zinc-800 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-800 text-center">
                      ...
                      <div className="text-[10px] text-zinc-500">Avg Test Score</div>
                    </div>
                    <div className="bg-zinc-50 dark:bg-zinc-800 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-800 text-center">
                      <div className="text-lg font-bold text-red-600 dark:text-red-400">
                        {candidates.reduce((sum, c) => sum + (c.cheatStrikes || 0), 0)}
                      </div>
                      <div className="text-[10px] text-zinc-500">Total Proct. Alerts</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="text-zinc-400 text-sm text-center italic">No assessment test activity recorded yet.</div>
              )}
            </CardContent>
          </Card>
        </div>
        ...
        {/* Data Table */}
        <Card className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-2xl overflow-hidden">
          <Table>
            <TableHeader className="bg-zinc-50 dark:bg-zinc-800">
              <TableRow>
                <TableHead className="py-4 font-semibold text-zinc-700 dark:text-zinc-300">Candidate Info</TableHead>
                <TableHead className="font-semibold text-zinc-700 dark:text-zinc-300">Target Role</TableHead>
                <TableHead className="font-semibold text-zinc-700 dark:text-zinc-300">Algorithmic Score</TableHead>
            ...
            <TableBody>
              {filteredCandidates.map((c) => (
                <TableRow key={c.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors cursor-pointer" onClick={() => {
                  ...
                  <TableCell>
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-rose-100 dark:bg-rose-950 text-rose-800 dark:text-rose-400">
                      {c.domain || 'Unspecified'}
                    </span>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="font-mono font-bold text-lg text-zinc-700 dark:text-zinc-300">
                  ...
                  <TableCell>
                    <Badge className={`px-2 py-1 rounded-md ${c.status === 'Shortlisted' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-400 hover:bg-emerald-200' : c.status === 'Rejected' ? 'bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-400 hover:bg-red-200' : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300 hover:bg-zinc-200'}`}>
                      {c.status}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right space-x-2" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" variant="outline" className="border-emerald-200 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400" onClick={() => updateStatus(c.id, 'Shortlisted')}>
                      <CheckCircle2 className="w-4 h-4 mr-1" /> Shortlist
                    </Button>
                    <Button size="sm" variant="outline" className="border-red-200 hover:bg-red-50 dark:hover:bg-red-950/30 text-red-700 dark:text-red-400" onClick={() => updateStatus(c.id, 'Rejected')}>
